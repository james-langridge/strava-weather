...
export class WeatherService {
    ...
    async getWeatherForActivity(
        ...
        try {
            const now = new Date();
            const hoursSinceActivity = (now.getTime() - activityTime.getTime()) / (1000 * 60 * 60);

            let weatherData: WeatherData;
            let dataSource: string;

            if (hoursSinceActivity > WEATHER_CONFIG.RECENT_ACTIVITY_THRESHOLD_HOURS &&
            ...
            } else if (hoursSinceActivity <= WEATHER_CONFIG.RECENT_ACTIVITY_THRESHOLD_HOURS) {
                ...
                logger.warn('Activity outside Time Machine range, using current weather', {
                    ...logContext,
                    hoursSinceActivity,
    ...
    private formatWeatherData(data: any): WeatherData {
        return {
            ...
            visibility: Math.round((data.visibility || WEATHER_CONFIG.DEFAULT_VISIBILITY_M) / 1000), // Convert to km
    ...
    private getCacheKey(lat: number, lon: number, time: Date, activityId: string): string {
        ...
        const minutes = Math.floor(roundedTime.getMinutes() / WEATHER_CONFIG.TIME_ROUND_MINUTES) *
